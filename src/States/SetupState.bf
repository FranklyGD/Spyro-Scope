	...
	class SetupState : WindowState {
		...
		public override void DrawGUI(Renderer renderer) {
			...
			if (Emulator.emulator == .None) {
				...
				WindowApp.font.Print(emulator, .(middleWindow - halfWidth, baseline, 0), .(255,255,255), renderer);
			}

			var baseline = (WindowApp.height - WindowApp.font.height) / 2;
			let halfWidth = WindowApp.font.CalculateWidth(message) / 2;
			WindowApp.font.Print(message, .(middleWindow - halfWidth, baseline, 0), .(255,255,255), renderer);
