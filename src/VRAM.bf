...
namespace SpyroScope {
	static struct VRAM {
		...
		public static void TakeSnapshot() {
			...
			Windows.ReadProcessMemory(Emulator.active.processHandle, (void*)(int)Emulator.active.VRAMBaseAddress, &snapshot[0], 1024 * 512 * 2, null);
		...
		public static void Write(uint16[] buffer, int x, int y, int width, int height) {
			...
			for (let localy < height) {
				...
				Windows.WriteProcessMemory(Emulator.active.processHandle, (void*)(int)(Emulator.active.VRAMBaseAddress + (x + (y + localy) * 1024) * 2), &buffer[(int)(localy * width)], width * 2, null);
			}
